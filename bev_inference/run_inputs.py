#!/usr/bin/env python3
"""
BEV Fusion inference on user-supplied images.

Reads every image (*.png, *.jpg, *.jpeg) from the ``inputs/`` folder,
...
Usage
-----
    python run_inputs.py                        # default weights
    python run_inputs.py --weights /path/to.pt  # custom checkpoint
    python run_inputs.py --install-deps         # pip-install first
"""
...
def main():
    parser = argparse.ArgumentParser(description="BEV Fusion — run on user-supplied images in inputs/")
    ...
    if not image_paths:
        ...
        print("       Place .png / .jpg images in the inputs/ folder and re-run.")
        sys.exit(1)
