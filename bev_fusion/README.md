...
Stage 3 of the pipeline: fusing detection + depth into a bird's-eye-view.
...
| File | Description |
|---|---|
| `bev_fusion_1.ipynb` | The BEV fusion notebook (detection + depth → 3D unprojection → BEV render) |
| `bev_outputs/` | Sample rendered outputs (RGB+detections, depth, BEV, composite) |
...
4. Use the KITTI camera intrinsics `K` to unproject pixel → 3D `(X, Y, Z)`
...
> The logic in this notebook is packaged into reproducible command-line scripts in [`../bev_inference/`](../bev_inference/), which run the same fusion on custom images, KITTI sequences, or videos and export GIFs + validation statistics.
